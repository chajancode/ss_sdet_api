...
## API1. CRUD через REST API со сверкой в БД (WordPress)
...
## API2. Получение данных через API + подготовка тест-данных в БД

- API2-1 Список постов без параметров (5 шт., все `publish`) -> [`test_get_posts_from_db.py::test_get_all_created_posts`](../tests/test_posts/test_get_posts_from_db.py)
- API2-2 Получение поста по id -> [`test_get_post_by_id`](../tests/test_posts/test_get_posts_from_db.py)
- API2-3 Несуществующий пост: 404 `rest_post_invalid_id` -> [`test_get_post_not_exists`](../tests/test_posts/test_get_posts_from_db.py)
- API2-4 / API2-5 Фильтрация постов по статусу `publish` и `draft` (параметризация) -> [`test_get_post_by_status`](../tests/test_posts/test_get_posts_from_db.py)
- API2-6 Список комментариев к посту (5 шт., `approved`) -> [`test_get_comms_from_db.py::test_all_created_comms`](../tests/test_comments/test_get_comms_from_db.py)
- API2-7 Получение комментария по id -> [`test_get_comm_by_id`](../tests/test_comments/test_get_comms_from_db.py)
- API2-8 Несуществующий комментарий: 404 `rest_comment_invalid_id` -> [`test_get_comm_not_exists`](../tests/test_comments/test_get_comms_from_db.py)
- API2-9 / API2-10 Фильтрация комментариев по статусу `approved` и `trash` (параметризация) -> [`test_get_comm_by_status`](../tests/test_comments/test_get_comms_from_db.py)
...
## API3. Автотесты REST (Яндекс.Диск, тест-кейсы №1 и №2)
...
## API4. Папки Яндекс.Диска (создание / удаление / восстановление)
...
## API5. Параллельный запуск тестов (pytest-xdist)
...
## API6. Полная валидация (Яндекс.Диск, тест-кейсы №3 и №4)
