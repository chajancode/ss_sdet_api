...
class CommentsService(BaseService):
    ...
    Args:
        ...
        (username/password).
        repository (CommentsRepository): Репозиторий комментариев.
    """
    def __init__(
            self, auth_data: dict, repository: CommentsRepository
            ) -> None:
        """
        Инициализирует сервис комментариев.

        Передаёт аутентификацию, эндпоинт в родительский класс BaseService.

        Args:
            auth_data (dict): Словарь с полями 'username' и 'password'.
            repository (CommentsRepository): Репозиторий комментвриев.
        """
        super().__init__(wpe.COMMENTS_ENDPOINT, auth_data)
